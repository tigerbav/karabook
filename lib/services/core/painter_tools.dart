...
class PainterTools {
  static final dbProvider = DBProvider.db;

  ///Сортировать список с областями по цветам
  static Map<Color, List<SvgShapeModel>> setSortedShapes(
      List<SvgShapeModel> shapes) {
  ...
  static FittedSizes getFittedSize(BuildContext context, String svgString) {
    final svgAttributes =
        XmlDocument.parse(svgString).findElements('svg').first.attributes;
    //debugPrint(svgAttributes.toString());

    final svgSize = Size(
      double.parse(
        svgAttributes
            .firstWhere((attribute) => attribute.name.toString() == 'width')
            .value,
      ),
      double.parse(
        svgAttributes
            .firstWhere((attribute) => attribute.name.toString() == 'height')
            .value,
      ),
    );
  ...
  static void setLinesAndShapes(BuildContext context, String svgString,
      List<SvgShapeModel> shapes, List<SvgLineModel> lines, FittedSizes fs) {
    //final fs = getFittedSize(context, svgString);
    final stringSvgPathShapes = List<XmlElement>.from(
        XmlDocument.parse(svgString).findAllElements('path'));
    final canvasSize = Size(
      MediaQuery.of(context).size.width,
      MediaQuery.of(context).size.height * 0.85,
    );
    final r =
        Alignment.center.inscribe(fs.destination, Offset.zero & canvasSize);
  ...
  static Future<void> getDbPainter(int id, List<SvgShapeModel> shapes,
      PainterProgressModel painterProgress) {
    return PainterTools.dbProvider.getPainter(id).then((painter) async {
      //Если прогресс существует, то записываем его в shapes
      if (painter!.shapes.isNotEmpty) {
        debugPrint(painter.id.toString());
        final dbShapesStringList = painter.shapes.split(' ');
        final shapesList =
            dbShapesStringList.map((e) => e.split(',')[1]).toList();
